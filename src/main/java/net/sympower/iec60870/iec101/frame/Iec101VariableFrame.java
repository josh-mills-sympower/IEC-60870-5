...
public class Iec101VariableFrame extends Iec101Frame {
    ...
    public int encode(byte[] buffer, IEC60870Settings settings) {
        ...
        System.out.println("variable frame asu length " + asduLength);
        ...
        System.out.println("controlFieldPos " + controlFieldPos);
        System.out.println("asduLength " + asduLength);
        System.out.println("checksum start " + controlFieldPos);
        System.out.println("checksum length " + (1 + addressLength + asduLength));
        System.out.println("checksum end pos " + (controlFieldPos + 1 + addressLength + asduLength - 1));

        // Calculate checksum for control + address + ASDU
        System.out.println("buffer before checksum: ");
        for (int i = 0; i < pos; i++) {
            System.out.printf("pos[%d]: 0x%02X", i, buffer[i] & 0xFF);
        }
        System.out.println();
        byte checksum = calculateChecksum(buffer, controlFieldPos, 1 + addressLength + asduLength);
        buffer[pos++] = checksum;
        System.out.println("buffer after checksum: ");
        for (int i = 0; i < pos; i++) {
            System.out.printf("pos[%d]: 0x%02X", i, buffer[i] & 0xFF);
        }
        System.out.println();
        ...
        System.out.println("variblae frame end pos " + pos);
        return pos;
    }

    private int getControlField() {
        System.out.println("DEBUG ENCODED CONTROL FIELD");
        System.out.println("Function Code " + functionCode.getCode());
        System.out.println("PRM " + prm);
        System.out.println("FCB " + fcb);
        System.out.println("FCV " + fcv);
